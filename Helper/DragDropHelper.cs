...
{
    //https://stackoverflow.com/a/1095445
    //https://github.com/bstollnitz/old-wpf-blog/blob/master/46-DragDropListBox/DragDropListBox/InsertionAdorner.cs
	//modify to handle tabcontrol
    ...
    public class DragDropHelper
	{
		...
		private DDDraggedAdorner draggedAdorner;
		private DDInsertionAdorner insertionAdorner;
		...
		private void DragSource_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
		{
			...
			//this is the code that take the tabitem when the event trigger in tabcontrol
		...
		private void DragSource_PreviewMouseMove(object sender, MouseEventArgs e)
		{
			if (this.draggedData != null)
			{
				...
				if (DDUtilities.IsMovementBigEnough(this.initialMousePosition, e.GetPosition(this.topWindow)))
				{
        ...
        private void DropTarget_PreviewDrop(object sender, DragEventArgs e)
        {
            ...
            if (draggedItem != null)
            {
				...
				//must go to main class to update list or it will be in mess up order
				//and it will not keep the cache too
				TabControl tabControl = this.targetItemsControl as TabControl;
				if (tabControl != null)
				{
		...
		private void DecideDropTarget(DragEventArgs e)
		{
			...
			if (IsDropDataTypeAllowed(draggedItem))
			{
				...
				{
					this.hasVerticalOrientation = DDUtilities.HasVerticalOrientation(this.targetItemsControl.ItemContainerGenerator.ContainerFromIndex(0) as FrameworkElement);
					...
					if (this.targetItemContainer != null)
					{
						...
						this.isInFirstHalf = DDUtilities.IsInFirstHalf(this.targetItemContainer, positionRelativeToItemContainer, this.hasVerticalOrientation);
		...
		private void CreateInsertionAdorner()
		{
			if (this.targetItemContainer != null)
			{
				...
				this.insertionAdorner = new DDInsertionAdorner(this.hasVerticalOrientation, this.isInFirstHalf, this.targetItemContainer, adornerLayer);
			}
		}
    ...
    public static class DDUtilities
    {
        ...
        public static bool HasVerticalOrientation(FrameworkElement itemContainer)
        {
            ...
            if (itemContainer != null)
            {
                ...
                else if ((stackPanel = panel as StackPanel) != null)
                {
		...
		//add parameter index, so can remove after add tab first to save the cache
        public static int RemoveItemFromItemsControl(ItemsControl itemsControl, object itemToRemove, int indexToBeRemoved)
        {
    ...
    public class DDDraggedAdorner : Adorner
    {
        ...
        public DDDraggedAdorner(object dragDropData, DataTemplate dragDropTemplate, UIElement adornedElement, AdornerLayer adornerLayer)
            : base(adornedElement)
        {
            ...
            //this.contentPresenter.ContentTemplate = dragDropTemplate; //commented because no template assign
        ...
        public void SetPosition(double left, double top)
        {
			...
			{
				//not sure yet what this do, but most likely keep get error because template is empty
    ...
    public class DDInsertionAdorner : Adorner
    {
        ...
        public DDInsertionAdorner(bool isSeparatorHorizontal, bool isInFirstHalf, UIElement adornedElement, AdornerLayer adornerLayer)
            : base(adornedElement)
        {
